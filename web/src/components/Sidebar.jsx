...
export default function Sidebar({ games, selectedGame, onSelectGame, activeView, onNavigate, onOpenOptions, onClose }) {
  ...
  const handleNav = (view) => {
    onNavigate(view)
    if (onClose) onClose() // close on mobile after selecting
  }
    ...
    <div style={styles.sidebar}>
      ...
      <div style={styles.gameHeader}>
        {selectedGame ? (
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '8px 0' }}>No game selected</div>
        )}
      </div>
          ...
          {games.map(game => {
            ...
            return (
              <button key={game.id}
                ...
                onClick={() => { onSelectGame(game); if (onClose) onClose() }}
                title={game.title}>
        ...
        {NAV_ITEMS.map(({ id, label, Icon, shortcut }) => {
          ...
          return (
            <button key={id}
              ...
              <span className="nav-shortcut" style={styles.navShortcut}>{shortcut}</span>
            </button>
          )
        })}
      </nav>
      ...
      <button style={styles.optionsBtn} onClick={() => { onOpenOptions(); if (onClose) onClose() }} title="Options (Ctrl+,)">
        ...
        <span className="nav-shortcut" style={styles.navShortcut}>Ctrl+,</span>
      </button>
    </div>
  )
}
...
const styles = {
  sidebar: { width: '100%', background: 'var(--bg-secondary)', display: 'flex', flexDirection: 'column', overflow: 'hidden', height: '100%' },
  ...
  gameItem: { display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: 'transparent', color: 'var(--text-secondary)', textAlign: 'left', fontSize: 12, cursor: 'pointer', border: 'none', borderLeft: '3px solid transparent', width: '100%', fontWeight: 500 },
  ...
  navItem: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', border: 'none', borderLeft: '3px solid transparent', fontSize: 13, textAlign: 'left', width: '100%', fontWeight: 500 },
  ...
  spacer: { flex: 1 },
  optionsBtn: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer', border: 'none', borderTop: '1px solid var(--border-color)', borderLeft: '3px solid transparent', fontSize: 13, width: '100%' },
}
